...
class CaseTimelinessMetricImporter extends Importer
{
    ...
    public static function getColumns(): array
    {
        return [
            ImportColumn::make('case_type')
                ->label('Case Type (REM, HOA or Appealed)')
                ->requiredMapping()
                ->rules(['required', 'max:30', Rule::in(array_column(CaseType::cases(), 'value'))]),
            ImportColumn::make('total_disposed')
                ->label('Total Disposed')
                ->requiredMapping()
                ->numeric()
                ->rules(['required', 'integer', 'min:0']),
            ImportColumn::make('total_ripe')
    ...
    public function resolveRecord(): ?CaseTimelinessMetric
    {
        ...
        return CaseTimelinessMetric::firstOrNew([
    ...
    {
        $body = 'Your case timeliness metric data import has completed and ' . number_format($import->successful_rows) . ' ' . str('row')->plural($import->successful_rows) . ' imported.';
