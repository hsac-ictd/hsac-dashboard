...
class IndigentLitigantImporter extends Importer
{
    ...
    public static function getColumns(): array
    {
        return [
            ...
            ImportColumn::make('total_indigents')
                ->label('Total Indigent Litigants')
                ->requiredMapping()
                ->numeric()
                ->rules(['required', 'integer', 'min:0']),
            ImportColumn::make('with_certificate')
                ->label('Total Indigent Litigants with Certificate')
                ->requiredMapping()
                ->numeric()
                ->rules(['required', 'integer', 'min:0']),
        ];
    ...
    public function resolveRecord(): ?IndigentLitigant
    {
        ...
        return IndigentLitigant::firstOrNew([
    ...
    {
        $body = 'Your indigents data import has completed and ' . number_format($import->successful_rows) . ' ' . str('row')->plural($import->successful_rows) . ' imported.';
